...
template<typename T> // 변수 타입이 정해지지 않았을때이기 때문에 템플릿 설정
class Node{
	public:
		//데이터를 저장할 변수
		T data;
...
void LinkedList<T>::Insert(T element){
	//노드 생성
	Node<T> *newnode = new Node<T>(element); // 템플릿 클래스로 만든 클래스 변수를 선언할때 반드시 <T> 를 붙여야함.
	...
	if (current_size != 0){
		for( i = first; i != NULL; i=i->link){
			if (index == current_size){
				cout << "[" << index << "|";
				cout << i -> data <<"]"; // 마지막 출력이기 때문에 "->" 를 지워줌.
			}
